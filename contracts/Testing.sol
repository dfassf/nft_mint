...
contract Testing is ERC721, ERC721Enumerable, ERC721Metadata, MinterRole {
    ...
    uint256 private _mintPrice;                   // 1 eth = 1000000000000000000
      ...
      returns (string memory)
    {
      ...
      if(revealed == false) {
        ...
        return bytes(currentNotRevealedUri).length > 0
            ? string(abi.encodePacked(currentNotRevealedUri, Strings.uint2str(tokenId), ".json"))
            : "";
      }
      string memory currentBaseURI = _baseURI();
      return bytes(currentBaseURI).length > 0
          ? string(abi.encodePacked(currentBaseURI, Strings.uint2str(tokenId), ".json"))
          : "";
    }
    ...
    address[] private whitelistedUsersArray;

    function addWhitelistedUsers(address[] calldata whitelistedUser) external onlyMinter{
        whitelistedUsersArray = whitelistedUser;
    }

    function getWhitelistedUsersArray() public view returns(address[] memory) {
        return whitelistedUsersArray;
    }
